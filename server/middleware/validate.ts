...
const VALID_CATEGORIES = ["Adventure", "Culture", "Food", "Nature", "City"] as const;
const SLUG_PATTERN = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;

export interface NormalizedPostBody {
  title: string;
  slug: string;
  excerpt: string;
  content: string;
...
export type PostBodyValidationResult =
  | { ok: true; body: NormalizedPostBody }
...
export function normalizePostBody(input: unknown): PostBodyValidationResult {
  const errors: string[] = [];
  ...
  let normalizedCategory: NormalizedPostBody["category"] = "";

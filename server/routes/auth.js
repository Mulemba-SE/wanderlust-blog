...
import { findAdminByEmail, findAdminByResetToken, countAdmins, createAdmin, updateAdminPassword, setAdminResetToken, clearAdminResetToken, } from "../db/queries/auth";
...
router.post("/auth/forgot-password", async (req, res) => {
    try {
        ...
        res.json({
            message: "If that email exists, a password reset link has been sent. Check your email.",
        });
    }
    catch (err) {
...
router.post("/auth/login", async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: admin.id, email: admin.email, role, name: admin.name }, process.env.JWT_SECRET, { expiresIn: "8h" });
        res.json({ token });
    }
    catch (err) {

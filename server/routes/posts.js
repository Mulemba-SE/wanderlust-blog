...
import { getAllPosts, getFeaturedPosts, getPostBySlug, searchPosts, createPost, updatePost, deletePost, } from "../db/queries/posts";
...
router.post("/posts", requireAuth, requireRole("admin", "editor"), validatePost, async (req, res) => {
    try {
        const { title, slug, excerpt, content, cover, category, authorId, publishedAt, readTime, featured = false, tags = [] } = req.body;
